...
0. **00 → 01**: NumPy/Pandas/PyTorch가 처음이거나 오래 안 써봤다면 **00**부터 손에 익힙니다(익숙하다면 건너뛰어도 됩니다). **01**은 이론 없이 scikit-learn 파이프라인을 한 바퀴 돌려보며 "머신러닝 코드가 대충 어떻게 생겼는지" 감을 잡는 워밍업입니다.
1. **02 → 03**: 지도학습의 기본 축인 회귀와 분류를 손코드(numpy)로 먼저 이해하고, scikit-learn으로 검증합니다. **이론은 여기서 시작합니다.**
