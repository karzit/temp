...
|---|---|---|---|
| [`crawl-storage-example`](crawl-storage-example) | A-1 | 웹 크롤링 → DB 원본 보관 | `requests`, `beautifulsoup4`, `psycopg2` |
| [`preprocess-example`](preprocess-example) | A-2 | DB 원본 → 청킹 → 벡터 인덱싱 | `langchain-text-splitters`, `PyMuPDF`, `langchain-openai` |
| [`document-input-example`](document-input-example) | B | 이미지 OCR → LLM 정형 출력 | `google-cloud-vision`, `openai`, `pydantic`, `streamlit` |
| [`rag-regulation-example`](rag-regulation-example) | C | 벡터 검색 → LLM 응답(RAG) | `langchain`, `opensearch-py`, `openai` |
