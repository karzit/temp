...
**어디서 막혔는지 알려주는 스크립트가 있습니다.** 아래 순서를 그대로 실행하면서
단계마다 데이터가 실제로 들어갔는지 확인합니다. 프로젝트가 넷이라 "안 되는데요"의 원인이
크롤링인지 DB인지 인덱싱인지 키인지 구분하기 어려운데, 그 구분을 대신해줍니다.

```bash
python example-projects/run_all.py --check   # 지금 무엇이 준비됐는지만 진단
python example-projects/run_all.py           # 1 -> 2 -> 3 순서대로 실행
```

`--check`는 아무것도 실행하지 않고 `.env`·PostgreSQL·OpenSearch·API 키 상태만 보여주므로,
아래 준비를 어디까지 했는지 확인할 때 먼저 돌려보면 됩니다.
직접 한 줄씩 따라가고 싶다면 아래 순서를 그대로 쓰세요 (스크립트가 하는 일과 같습니다).
