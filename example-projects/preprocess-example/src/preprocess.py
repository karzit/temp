"""PostgreSQL 원본 -> 500자 청킹 -> OpenSearch 인덱싱까지 수행하는 메인 스크립트입니다.

rag-regulation-example의 ingest.py와 큰 흐름은 같습니다(원본 -> 청킹 -> 임베딩 -> OpenSearch).
다른 점은 두 가지입니다.
    - 원본을 어디서 가져오는가
    ...
    - 무슨 기준으로 자르는가
        · ingest.py    : parse.py로 조 표지를 찾아 조항 단위로 자름
        · preprocess.py: 크롤링 문서라 조 표지를 기대할 수 없어 고정 길이(500자)로 자름
...
# rag-regulation-example은 조 표지를 파싱해서 조항 단위로 자르지만(parse.py + ingest.py),
# 여기는 웹에서 긁어온 문서라 그 방법을 쓰기 어렵습니다. 사이트마다 조 표지를 <table>이나
# <div>로 흩어놓거나 아예 안 쓰기도 해서, 파서가 조를 못 찾거나 엉뚱하게 자를 위험이 큽니다.
# 그래서 여기서는 형식에 기대지 않는 고정 길이로 자릅니다.
#
# 다만 그 프로젝트의 폴백 크기(1000자)보다는 작게 잡았습니다. 규정 문서는 조 하나가 짧아서
# 1000자로 자르면 한 청크에 관련 없는 조가 여러 개 섞이는데, 그러면 검색은 걸려도
# "질문과 정확히 관련된 조항 하나"를 집어내기 어려워지기 때문입니다.
CHUNK_SIZE = 500
CHUNK_OVERLAP = 75  # CHUNK_SIZE가 작아진 만큼 겹침도 비례해서 줄였습니다 (폴백 1000자 대비 150자 = 15% 비율 유지)

# Kiwi는 초기화(사전 로딩)에 비용이 좀 들어서, 문서마다 새로 만들지 않고 모듈 레벨에서 한 번만 만듭니다.
_kiwi = Kiwi()


def clean_text(text: str) -> str:
    """크롤링/OCR로 얻은 원문의 잡음을 정규식으로 정리합니다.

    HTML을 텍스트로 바꾸거나 PDF에서 글자를 뽑다 보면 연속된 공백, 과도한 빈 줄,
    반복된 특수문자(마침표·느낌표 등)가 섞여 들어오는 경우가 흔합니다. 청킹 전에 한 번
    정리해두면 청크 경계가 잡음 때문에 이상하게 갈리는 것을 줄일 수 있습니다.
    자세한 설명과 실습은
...
def extract_keywords(text: str, limit: int = 10) -> list[str]:
    """형태소 분석으로 명사(NNG/NNP)만 뽑아 청크의 대표 키워드로 남깁니다.

    "휴가를"/"휴가가"처럼 조사가 붙어도 형태소 분석을 거치면 같은 명사("휴가")로 인식되기
    때문에, 공백 기준 분리보다 안정적인 키워드 태깅이 가능합니다. 등장 빈도가 높은 순으로
    최대 `limit`개만 남깁니다. 자세한 설명과 실습은 위 노트북의 실습 9 참고.

    이 값이 실제로 쓰이는 곳은 rag-regulation-example의 `search_keyword_docs()`입니다.
    본문("text")과 이 필드("metadata.keywords")를 함께 검색하는데, 왜 그게 한국어 검색에
    도움이 되는지는 그 함수의 docstring에 정리해뒀습니다.
    """
...
def extract_pdf_text(binary_content: bytes) -> str:
    """PDF 파일의 이진 데이터를 사람이 읽는 텍스트로 변환합니다.

    PyMuPDF(모듈명 fitz)는 PDF 페이지를 하나씩 열어서 그 안의 글자를 뽑아주는 도구입니다.
    파일을 디스크에 저장하지 않고, 메모리에 있는 바이트(binary_content) 그대로 열 수 있어서 편리합니다.
    """
    text_parts = []
    # io.BytesIO: 파일이 아니라 메모리 상의 바이트 덩어리를 "파일인 척" 다룰 수 있게 해주는 도구입니다.
...
def raw_document_to_text(raw_doc: RawDocument) -> str:
    """RawDocument 하나를 (PDF든 HTML이든 상관없이) 정제된 순수 텍스트로 통일해서 반환합니다."""
...
def split_into_chunks(raw_docs: list[RawDocument]) -> list[Document]:
    """원본 문서들을 500자 청크(LangChain Document 객체)로 잘라줍니다."""
    ...
    for raw_doc in raw_docs:
        ...
        if not text.strip():
            # 텍스트를 하나도 못 뽑아낸 문서(빈 PDF 등)는 건너뜁니다.
            print(f"건너뜀 (텍스트 없음): {raw_doc.url}")
            continue

        # LangChain의 텍스트 분할기는 Document 객체 목록을 입력으로 받으니,
        # 순수 텍스트를 먼저 Document로 감싸줍니다. metadata에 출처 URL을 함께 담아둡니다.
        doc = Document(page_content=text, metadata={"source": raw_doc.url})
        chunks = splitter.split_documents([doc])
        for chunk in chunks:
            # 청크별 대표 명사를 메타데이터로 남겨둡니다.
            # 저장만 하고 끝나는 값이 아니라는 점은 extract_keywords()의 docstring 참고.
...
def index_chunks(chunks: list[Document]) -> None:
    """청크들을 임베딩으로 변환해 OpenSearch에 저장합니다. (rag-regulation-example/ingest.py와 동일한 방식)"""
