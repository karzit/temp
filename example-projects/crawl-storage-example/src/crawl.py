"""targets.py에 적힌 URL들을 하나씩 방문해서 PostgreSQL에 저장하는 메인 스크립트입니다.
...
사진을 찍거나(PDF는 그대로 복사) 내용을 옮겨 적어(HTML은 텍스트만 추출) 우리 창고(PostgreSQL)에 쌓는 일입니다.
...
# 일부 사이트는 "브라우저가 아닌 요청"을 차단하기도 해서, 브라우저인 척하는 헤더를 붙여줍니다.
HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; regulation-crawler/1.0)"}


def fetch(url: str) -> requests.Response:
    """URL 하나에 접속해서 응답을 받아옵니다. 실패하면 예외를 던집니다 (raise_for_status)."""
    response = requests.get(url, headers=HEADERS, timeout=10)
    response.raise_for_status()  # 200(성공)이 아니면 여기서 에러를 발생시켜 실패를 바로 알아챌 수 있게 합니다.
    return response


def extract_text_from_html(html: str) -> str:
    """HTML 문서에서 사람이 읽는 "본문 글자"만 뽑아냅니다.

    HTML에는 <script>(자바스크립트 코드)나 <style>(디자인 코드)처럼
    우리가 읽을 필요 없는 내용도 섞여 있어서, 이런 건 미리 제거하고 순수 텍스트만 남깁니다.
    """
    ...
    # get_text()는 태그를 다 떼어내고 글자만 이어붙여줍니다.
    # separator="\n"으로 태그 경계마다 줄바꿈을 넣어서 문단 구분이 아예 사라지지 않게 합니다.
...
def crawl_one(url: str) -> None:
    """URL 하나를 크롤링해서 DB에 저장하는 한 사이클입니다."""
    print(f"크롤링 중: {url}")
    response = fetch(url)

    if url.lower().endswith(".pdf"):
        # PDF는 텍스트로 바꾸지 않고 원본 바이너리 그대로 저장합니다.
        # (PDF 안의 글자를 뽑아내는 작업은 다음 단계인 preprocess-example의 몫입니다)
...
def main():
    init_db()  # 테이블이 없으면 먼저 만들어둡니다.

    for url in CRAWL_TARGETS:
        try:
            crawl_one(url)
        except requests.RequestException as e:
            # 사이트 하나가 실패해도 전체 크롤링이 멈추지 않도록, 에러만 출력하고 다음 URL로 넘어갑니다.
            print(f"실패: {url} ({e})")

        # 다음 요청 전에 잠깐 쉬어서 상대 서버에 부담을 주지 않습니다.
