"""OCR로 뽑아낸 지저분한 텍스트를, AI를 시켜서 깔끔한 JSON 형태로 정리하는 부분입니다.
...
def structure_text(raw_text: str) -> RegulationInquiry:
    """OCR 원문 텍스트를 받아서 RegulationInquiry 형태의 정형 데이터로 바꿔줍니다."""
    client = OpenAI(api_key=OPENAI_API_KEY)

    # beta.chat.completions.parse: "이 Pydantic 모델(RegulationInquiry) 형식에 딱 맞춰서 답해줘"라고
    # OpenAI에게 요청하는 기능입니다. 응답이 자동으로 RegulationInquiry 객체로 변환되어 돌아옵니다.
    # (사람이 직접 "response.choices[0]...에서 JSON 파싱해서..." 하는 번거로운 과정을 대신 해줍니다)
