...
# 내부적으로 이 이름의 환경변수를 자동으로 찾아서 인증에 사용합니다.
# load_dotenv()가 .env 파일 값을 환경변수로 등록해주기 때문에, import만 해도 인증 준비가 끝납니다.
# GOOGLE_APPLICATION_CREDENTIALS=./google-vision-key.json
#
# 아래 검증이 없으면 이 값이 빠졌을 때 앱 실행 시점엔 아무 에러도 없다가, 사용자가 이미지를
# 업로드하고 "분석 시작"을 눌러야 ocr.py 안에서야 (그것도 알아보기 힘든 구글 SDK 에러 메시지로)
# 실패합니다. OPENAI_API_KEY처럼 시작하자마자 바로 알아챌 수 있도록 여기서 미리 확인해둡니다.
