"""검색이 실제로 잘 되고 있는지 점수로 재는 스크립트 (평가 하네스).

여기까지 오면 이런 상태야. 청킹 방식도 바꿔봤고, 하이브리드 검색도 붙였고, 리랭커도 달았어.
그런데 **정말 좋아진 걸까?** 질문 몇 개 던져보고 "오, 잘 나오네" 하는 걸로는 알 수 없어.
어제 잘 되던 질문이 오늘 안 되는 걸 눈치채지도 못하고.

그래서 필요한 게 이거야. 정답지(골든셋)를 미리 만들어두고, 설정을 바꿀 때마다
**같은 질문 세트로 채점해서 숫자를 비교**하는 것. 학습할 때 문제집을 풀고 채점하는 것과 같아.

이 스크립트가 재는 지표 두 개:

    hit@k   상위 k개 안에 정답 조항이 하나라도 들어 있던 질문의 비율.
            "AI에게 넘길 4개 안에 근거가 들어는 갔나?"를 본다. 이게 낮으면
            프롬프트를 아무리 다듬어도 소용없다. 애초에 자료가 안 들어간 거니까.

    MRR     정답이 몇 등으로 나왔는지를 점수로 바꾼 것. 1등이면 1점, 2등이면 1/2점,
            3등이면 1/3점... 이렇게 매기고 전체 질문에 대해 평균 낸다.
            hit@5가 같아도 MRR이 높으면 "정답을 더 위쪽에 올려놨다"는 뜻이다.
            (Mean Reciprocal Rank, 평균 역순위)

실행하면 검색 방식 4가지를 같은 질문으로 돌려서 표로 비교해준다:

    python src/evaluate.py

주의: 리랭커 열은 모델을 내려받아야 해서 처음엔 오래 걸린다.
      빼고 돌리려면 .env에 USE_RERANKER=false를 적으면 된다.
"""
import json
import sys
...
# 몇 등까지를 "찾았다"고 볼 것인가. 5로 두는 게 관행인데, 이유는 실용적이다.
# 보통 AI에게 4~5개 조각을 넘기니까, 상위 5개 안에 정답이 있으면 답변이 가능하다는 뜻이 된다.
K = 5
...
def hit_and_rank(docs: list[Document], gold_articles: list[str]) -> tuple[bool, int]:
    """검색 결과에서 정답 조항이 몇 등에 처음 나왔는지 찾는다.

    반환값은 (맞췄는지, 몇 등인지). 못 찾았으면 (False, 0).

    같은 조가 여러 조각으로 나뉘어 있어도(①②③) 조 번호가 같으면 같은 정답으로 친다.
    사람이 "제12조를 찾아왔네"라고 판단하는 기준과 맞추기 위해서다.
    """
...
def recall_at_k(docs: list[Document], gold_articles: list[str]) -> float:
    """정답 조항이 여러 개일 때, 그중 몇 퍼센트를 찾아왔는지.

    "집에서 일하는 날 몇 시간 일하나요?"처럼 제11조와 제9조를 둘 다 봐야 답이 되는
    질문에서, 하나만 찾아온 것과 둘 다 찾아온 것을 구분하기 위해 필요하다.
    """
...
# 똑같은 방식으로 채점할 수 있다. 새 검색 방식을 시험해보고 싶으면 이 모양에 맞춰
# 함수를 하나 더 만들고 STRATEGIES에 추가하면 된다.
...
def main() -> None:
    ...
    # 못 찾은 질문은 따로 보여준다. 점수보다 이게 더 쓸모 있을 때가 많다.
    # "왜 이 질문만 안 될까"를 파고들면 청킹이나 프롬프트에서 고칠 거리가 나온다.
    print("\n[놓친 질문]")
    for name, scores in results.items():
        if scores["misses"]:
            print(f"  {name}:")
    ...
    print(
        ...
        "\n      다시 색인한 뒤 이 스크립트를 돌리면, 그 변경이 점수를 올렸는지 내렸는지 알 수 있다."
    )
