"""규정 문서의 텍스트를 "장 -> 절 -> 조 -> 항" 계층 트리로 파싱하는 부분.

ingest.py가 원래 하던 일은 "1000자씩 뚝뚝 자르기"였어. 그런데 규정 문서에는
사람이 이미 정해둔 자연스러운 경계가 있어. 바로 **조항**이야.

왜 이게 중요한지 예를 들어볼게. "재택근무 중 연장근로 수당은 어떻게 되나요?"라는
질문의 답은 제11조 ③항에 있어. 그런데 1000자로 자르면 이런 일이 생겨:

    청크 A: "...제10조(휴게) ... 제11조(재택근무) ① 사원은 부서장의 승인을 받아
             주 2일의 범위에서 재택근무를 할 수 있다. ② 재택근무일의 소정근로시"
    청크 B: "간은 제9조 제2항을 준용한다. ③ 재택근무 중 발생한 연장근로에 대하여는..."

답이 청크 B에 있는데, 청크 B만 읽으면 이게 **무슨 조항인지 알 수가 없어**.
"제11조 재택근무"라는 제목이 청크 A에 있으니까. 검색도 안 잡히고, 잡혀도
AI가 출처를 "제11조 ③항"이라고 말해줄 수가 없지.

그래서 이 파일이 하는 일:
    1. 텍스트에서 "제1장", "제2절", "제11조(재택근무)", "①" 같은 표지를 찾아낸다
    2. 각 조가 어느 장/절에 속하는지 계층 경로를 만든다
       -> "제3장 근무 > 제11조(재택근무)"
    3. 조 단위로 자르되, 조가 너무 길면 항(①②③) 단위로 한 번 더 자른다
    4. 각 조각 앞에 계층 경로를 붙여준다 (프리픽스)
       -> 조각만 따로 떼어놔도 "이게 어디 얘기인지" 알 수 있게

비유하자면, 1000자 청킹이 "책을 자로 재서 똑같은 길이로 찢는 것"이라면
이건 "목차를 보고 절 단위로 뜯어서, 각 장에 포스트잇으로 제목을 붙여두는 것"이야.

단독 실행해서 결과를 눈으로 확인할 수 있어:
...
# ---------------------------------------------------------------------------
# 규정 문서는 형식이 꽤 정해져 있어서, 정규식만으로도 상당히 정확하게 잡힌다.
# LLM에게 구조를 파악시키는 방법도 있지만(03 노트북의 정형 출력처럼), 조 번호처럼
# "틀리면 안 되는" 정보는 규칙 기반이 더 안전하고 비용도 0이다.
#
# 정규식이 낯설다면 02 노트북 실습 8(정규식으로 텍스트 정제하기)을 먼저 보면 좋다.

# "제1장 총칙", "제2절 인사이동" 처럼 조보다 상위인 구분 단위.
# 편/장/절/관 순서로 좁아진다. 문서에 따라 일부만 쓰기도 한다.
SECTION_RE = re.compile(r"^제\s*(\d+)\s*(편|장|절|관)\s*(.*)$")

# "제11조(재택근무)", "제11조의2(원격근무 보안)" 처럼 조 표지.
#   - (\d+)      -> 조 번호 (11)
#   - (?:의\s*(\d+))?  -> "의2" 같은 가지번호. 없을 수도 있어서 ? 를 붙였다.
#   - \(([^)]*)\)      -> 괄호 안 제목. 이것도 없는 문서가 있어서 선택으로 뒀다.
ARTICLE_RE = re.compile(r"^제\s*(\d+)\s*조(?:\s*의\s*(\d+))?\s*(?:\(([^)]*)\))?")

# "①②③..." 항 번호. 규정 문서는 거의 원문자를 쓴다.
CIRCLED = "①②③④⑤⑥⑦⑧⑨⑩⑪⑫⑬⑭⑮"
...
@dataclass
class Article:
    """조 하나를 나타낸다. ingest.py가 이걸 받아서 청크로 만든다."""

    number: str  # "제11조" 또는 "제11조의2"
    sort_key: tuple[int, int]  # (11, 0) / (11, 2) — 번호 연속성 검증과 정렬에 쓴다
...
def parse_articles(pages: list[str]) -> list[Article]:
    """페이지별 텍스트 목록을 받아서, 조 단위 Article 목록으로 변환한다.

    pages는 PyPDFLoader가 준 페이지 텍스트 리스트를 그대로 넣으면 된다.
    (텍스트 파일 하나라면 [전체_텍스트]처럼 1개짜리 리스트로 넣어도 동작한다.)
    """
    # 페이지들을 하나로 이어붙이면서, 각 페이지가 몇 번째 글자에서 시작하는지 기록해둔다.
    ...
    # 지금까지 지나온 편/장/절/관을 (깊이, 표시이름) 쌍으로 쌓아둔다.
    # 깊이를 같이 들고 있어야 "제2장을 만나면 이전 장과 그 아래 절을 전부 비운다"를
    # 정확히 할 수 있다. (문서에 편이 없고 장부터 시작하는 경우가 많아서,
    # 리스트 위치를 깊이로 그냥 쓰면 어긋난다.)
    ...
    for line in text.split("\n"):
        ...
        # (2) 조 표지를 만나면 이전 조를 마감하고 새 조를 시작한다.
        ...
        if article_match:
            ...
            current = Article(
                ...
                path=[label for _depth, label in current_path],  # 지금 시점의 경로를 복사해둔다
                page=_page_of(line_start, page_starts),
            )
            body_lines = [stripped]
            continue

        # (3) 그 밖의 줄은 현재 조의 본문으로 쌓는다.
        #     조가 시작되기 전의 줄(문서 제목 등)은 버린다.
        if current is not None:
...
def _strip_heading(body: str) -> str:
    """본문 맨 앞의 조 표지("제12조(연장근로와 수당)")를 떼어낸다.

    표지는 ingest.py가 계층 경로 프리픽스로 따로 붙여주기 때문에, 본문에까지 남겨두면
    같은 문구가 두 번 들어가서 임베딩만 흐려진다.

    주의할 점: "제1조(목적) 이 규칙은 ..." 처럼 표지와 본문이 한 줄에 붙어 있는 경우가
    흔하다. 그래서 첫 줄을 통째로 버리면 안 되고, 표지에 해당하는 부분만 잘라내야 한다.
    """
...
def split_paragraphs(article: Article, max_chars: int = 900) -> list[tuple[str, str]]:
    """조 본문을 (필요하면 항 단위로 쪼개서) 돌려준다.

    반환값은 (항 표시, 본문) 튜플 목록이다. 항 표시가 없으면 빈 문자열.
    본문에서 조 표지는 빠져 있다 (_strip_heading 참고).

    짧은 조는 통째로 두는 게 문맥이 살아서 더 좋기 때문에, max_chars를 넘을 때만 쪼갠다.
    """
    ...
    # 원문자 앞에서 자른다. (?=...) 는 "자르되 그 글자는 남겨둬"라는 뜻(lookahead).
...
def check_article_sequence(articles: list[Article]) -> list[str]:
    """조 번호가 중간에 건너뛰거나 중복되지 않았는지 확인한다 (무결성 검증).

    PDF 텍스트 추출은 생각보다 자주 실패한다. 두 단 편집(2단 조판)이나 표 안에 들어간
    조문은 통째로 누락되기도 한다. 그런데 누락은 조용히 일어나기 때문에,
    검증하지 않으면 "챗봇이 그 조항만 유독 못 찾는" 상태로 서비스가 나간다.

    조 번호는 원래 1, 2, 3... 으로 이어지므로, 끊긴 지점을 찾으면
    "추출이 실패했을 가능성이 있는 위치"를 알 수 있다.
    """
    warnings: list[str] = []
    ...
    for article in sorted(articles, key=lambda a: a.sort_key):
        ...
        # 가지번호(제11조의2)는 본번호와 같은 자리이므로 연속성 검사에서 제외한다.
        if branch:
            continue
...
if __name__ == "__main__":
    # 파서가 뭘 뽑아냈는지 눈으로 확인하는 용도. OpenSearch도 OpenAI 키도 필요 없다.
    #     python src/parse.py data/sample_regulation.txt
    if len(sys.argv) < 2:
