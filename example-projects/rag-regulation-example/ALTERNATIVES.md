...
이 프로젝트는 `langchain` + `opensearch-py` + `pypdf` + `gpt-4o-mini`(openai) 조합으로 만들어졌습니다.
...
| 대안 | 언제 고려하나 |
|---|---|
| LLM 정형 출력 (`pydantic` + gpt-4o-mini) | 문서마다 조문 형식이 제각각이라 정규식으로 감당이 안 될 때. 다만 조 번호처럼 틀리면 안 되는 값은 규칙 기반이 더 안전하고 비용도 0입니다. `document-input-example`에서 쓰는 방식 |
...
**한 번은 직접 짜봐야 숫자가 뭘 뜻하는지 감이 오기 때문**입니다. 규모가 커지면 위 도구로 옮겨도 됩니다.
