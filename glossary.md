...
짧은 한국어 텍스트(상품명)를 카테고리로 분류할 때 등장하는 용어들.
`notebooks/text-classification-practice/`의 01~02 노트북에서 실습한다.

#### <a id="text-classification"></a>텍스트 분류 (Text Classification)
글을 미리 정해진 범주 중 하나로 나누는 문제. 스팸 판정, 감성 분석, 상품 카테고리 분류가 모두 여기에 속한다.
...
**어순이 사라진다** — "매운 치즈 라면"과 "치즈 매운 라면"이 같은 벡터가 된다.
상품명처럼 어순에 뜻이 없는 짧은 텍스트에서는 이 단순함이 오히려 장점이다. → [TF-IDF](#tfidf)

#### <a id="tokenization"></a>토큰화 (Tokenization)
문자열을 사전에 넣을 단위(토큰)로 쪼개는 일. **무엇을 토큰으로 삼느냐가 곧 피처 설계다.**
`CountVectorizer`의 기본값은 공백·문장부호로 자르고 두 글자 이상만 남긴다(`110g` 같은 토큰이 사라진다).
한국어는 조사가 붙어 `라면이`/`라면을`이 다른 토큰이 되므로, [형태소 분석](#morphological-analysis)이나
[문자 n-gram](#char-ngram)으로 완화한다.

#### <a id="char-ngram"></a>문자 n-gram (`analyzer="char_wb"`)
단어 대신 **글자 2~3개짜리 조각**을 세는 방식. `컵라면` → `컵라`, `라면`, `컵라면`.
`치즈라면`과 `치즈 라면`이 `라면` 조각을 공유하므로 **띄어쓰기 흔들림과 오타에 강하다.**
한국어에서 형태소 분석기를 붙이기 전에 먼저 시도해볼 만한 기본 선택지다. 대가는 피처 수 증가.

#### <a id="class-imbalance"></a>클래스 불균형 (Class Imbalance)
카테고리별 건수가 크게 다른 상태. 가장 많은 카테고리가 전체의 18%, 가장 적은 쪽이 3%라면
**정확도는 소수 카테고리의 실패를 감춘다.** 대응은 ① [macro f1](#macro-f1)을 함께 보기,
...
3%짜리 카테고리를 통째로 틀리면 크게 떨어진다. **불균형 데이터의 진짜 성능**을 볼 때 쓴다.

#### <a id="confusion-matrix"></a>혼동 행렬 (Confusion Matrix)
행은 실제, 열은 예측인 표. **대각선이 아니라 대각선 바깥을 보는 그림이다.**
어떤 카테고리 쌍이 서로 넘어가는지 찾으면, 그것이 데이터의 성격 때문인지(양쪽에 다 쓰이는 단어)
모델의 부족 때문인지 판단할 실마리가 된다.

#### <a id="label-noise"></a>라벨 오류 (Label Noise)
정답 자체가 틀린 행. 사람이 손으로 분류한 데이터에는 늘 섞여 있다.
**정확도 100%가 목표가 아닌 이유**다. 맞힐 수 없는 행까지 맞히는 모델은 라벨 오류를 외운
[과적합](#overfitting) 상태다. 같은 입력에 다른 라벨이 붙은 행은 라벨 오류의 직접적인 증거다.
...
#### <a id="oov"></a>OOV / `[UNK]`
사전에 없는 단어(Out-Of-Vocabulary). 예측 시 처음 보는 브랜드는 전부 `[UNK]`가 된다.
`max_tokens`를 실제 어휘보다 작게 잡으면 학습 중에도 `[UNK]`가 생겨, 처음 보는 단어에 강해지는 효과가 있다.

#### <a id="conv1d"></a>`Conv1D` (1차원 합성곱)
[CNN](#cnn)을 이미지가 아니라 **텍스트에 1차원으로** 적용한 것. 이미지에서 3×3 필터가 인접 픽셀을 보듯,
`Conv1D(128, 3)`은 **인접한 토큰 3개 묶음**에서 패턴을 찾는다. 글자 단위 입력에 쓰면 사실상 학습되는 문자 3-gram이다.

#### <a id="ensemble"></a>앙상블 (Ensemble)
여러 모델의 예측 확률을 평균 내어 최종 답을 정하는 방법.
**이득의 조건은 "모델들이 서로 다른 실수를 하는 것"이다.** 같은 데이터를 같은 방식으로 본 모델을
여러 개 더해봐야 소용이 없다. 둘 다 틀리는 행은 앙상블로도 구제되지 않는다.

#### <a id="keras-h5"></a>`.h5` vs `.keras` (모델 저장 포맷)
`.h5`는 숫자 배열만 담는 옛 포맷이라 **[`TextVectorization`](#text-vectorization)이 들고 있는 단어 사전을
복원하지 못한다.** 저장은 되지만 불러오기가 실패한다. 대응은 두 가지다 —
