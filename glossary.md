...
④ tabular-ml-practice, ⑤ text-classification-practice)에서 반복적으로 등장하는 용어를 모아둔 통합 용어집입니다.
...
- [7. 텍스트 분류 실무 (text-classification-practice)](#7-텍스트-분류-실무-text-classification-practice)
- [8. 사전 학습 모델 (text-classification-practice 03)](#8-사전-학습-모델-text-classification-practice-03)
- [9. Transformer / LLM 내부 구조 (ml-curriculum 08)](#9-transformer--llm-내부-구조-ml-curriculum-08)
...
## 7. 텍스트 분류 실무 (text-classification-practice)

짧은 한국어 텍스트(뉴스 제목)를 주제로 분류할 때 등장하는 용어들.
`notebooks/text-classification-practice/`의 01~02 노트북에서 공개 데이터셋 KLUE-YNAT로 실습한다.

#### <a id="text-classification"></a>텍스트 분류 (Text Classification)
글을 미리 정해진 범주 중 하나로 나누는 문제. 스팸 판정, 감성 분석, 뉴스 주제 분류가 모두 여기에 속한다.
표 데이터 분류와 다른 점은 **입력이 컬럼이 아니라 문자열 하나**라는 것이다. 그래서 피처를 직접 만들어야 한다.

#### <a id="bow"></a>BoW / 단어 가방 (Bag of Words)
문서에 나온 단어를 사전 순서대로 세어 벡터로 만드는 방법. `CountVectorizer`가 이것을 한다.
**어순이 사라진다** — "북한이 미국을 비판"과 "미국이 북한을 비판"이 같은 벡터가 된다.
뜻은 정반대지만, **주제**를 맞히는 데는 어떤 단어가 나왔는지가 더 중요해서 잘 통한다. → [TF-IDF](#tfidf)

#### <a id="tokenization"></a>토큰화 (Tokenization)
문자열을 사전에 넣을 단위(토큰)로 쪼개는 일. **무엇을 토큰으로 삼느냐가 곧 피처 설계다.**
`CountVectorizer`의 기본값은 공백·문장부호로 자르고 두 글자 이상만 남긴다(순수한 숫자 토큰이 사라진다).
한국어는 조사와 어미가 붙어 `정부는`/`정부가`/`열렸다`/`열린`이 전부 다른 토큰이 된다.
그래서 **사전이 폭증하고 단어별 등장 횟수는 줄어든다** — 뉴스 제목 16,000건에서 서로 다른 단어가
44,901개 나왔고 그중 74%가 딱 한 번만 등장했다. [형태소 분석](#morphological-analysis)이나
[문자 n-gram](#char-ngram)으로 완화한다.

#### <a id="char-ngram"></a>문자 n-gram (`analyzer="char_wb"`)
단어 대신 **글자 2~3개짜리 조각**을 세는 방식. `정부는` → `정부`, `부는`, `정부는`.
`정부는`과 `정부가`가 `정부` 조각을 공유하므로 **조사·어미·띄어쓰기 흔들림·오타에 강하다.**
한국어에서 형태소 분석기를 붙이기 전에 먼저 시도해볼 만한 기본 선택지다. 대가는 피처 수 증가.

실측: 뉴스 제목 주제 분류에서 단어 단위 0.7632 → 문자 2~3-gram **0.8455**.
인자 두 개로 얻은 +8%p라, 모델을 바꿔서 얻은 +0.004보다 스무 배 컸다.
**전처리의 좋고 나쁨도 여기에 딸려 바뀐다** — 같은 정제 함수가 단어 단위에서는 +0.004,
문자 단위에서는 −0.017이었다.

#### <a id="class-imbalance"></a>클래스 불균형 (Class Imbalance)
카테고리별 건수가 크게 다른 상태. **정확도는 건수가 적은 카테고리의 실패를 감춘다.**
심하지 않은 불균형(18% 대 11%)에서도 카테고리별 f1은 0.94에서 0.62까지 벌어질 수 있다.
대응은 ① [macro f1](#macro-f1)을 함께 보기,
② `class_weight="balanced"`로 소수 카테고리의 오답에 벌점을 키우기. ②는 전체 정확도를 조금 깎는 거래다.

#### <a id="macro-f1"></a>macro 평균 / macro f1
카테고리별 지표를 **건수와 무관하게 단순 평균**한 값. 건수로 가중하는 weighted 평균이나 정확도와 달리,
작은 카테고리를 통째로 틀리면 크게 떨어진다. **모든 카테고리를 동등하게 볼 때** 쓴다.

#### <a id="confusion-matrix"></a>혼동 행렬 (Confusion Matrix)
행은 실제, 열은 예측인 표. **대각선이 아니라 대각선 바깥을 보는 그림이다.**
어떤 카테고리 쌍이 서로 넘어가는지 찾으면, 그것이 데이터의 성격 때문인지 모델의 부족 때문인지
판단할 실마리가 된다. 오답이 **한 카테고리에 몰려 있으면** 두 범주의 경계를 다듬어 고칠 여지가 있고,
**전방위로 흩어져 있으면** 그 범주가 "딱 떨어지지 않는 것이 모이는 칸"(잔여 범주)이라는 신호다.

#### <a id="label-noise"></a>라벨 오류와 라벨 모호성 (Label Noise / Ambiguity)
**라벨 오류**는 정답 자체가 틀린 행이고, **라벨 모호성**은 사람이 봐도 정답이 갈리는 행이다.
사람이 손으로 분류한 데이터에는 둘 다 섞여 있다. **정확도 100%가 목표가 아닌 이유**다.
맞힐 수 없는 행까지 맞히는 모델은 그것을 외운 [과적합](#overfitting) 상태다.

**모호성의 크기를 재는 법.** 같은 입력에 다른 라벨이 붙은 행을 세는 것이 가장 쉬운 점검이고,
주석 기록이 남아 있다면 **주석자끼리 얼마나 갈렸는지**를 보면 된다. KLUE-YNAT에서는
주석자 세 명이 **36%에서 2:1로 갈렸다.** 그 숫자가 이 문제의 상한선을 알려준다.

#### <a id="paired-comparison"></a>짝지은 비교 (Paired Comparison)
두 방법 중 무엇이 나은지 판단할 때, **같은 분할·같은 seed에서 둘을 나란히 학습해 차이를 보는 것**.
절대 정확도는 분할에 따라 크게 흔들리므로(뉴스 제목 분류에서 폭 0.01) 한 번 재서 나온 0.001은
아무 의미가 없다. 같은 분할 안에서는 그 흔들림이 상쇄되므로, **여러 분할에서 차이의 방향이
일관되는지**를 보면 작은 효과도 판단할 수 있다.

#### <a id="distribution-shift"></a>분포 이동 (Distribution Shift)
학습 데이터와 실제로 들어오는 데이터의 분포가 다른 현상. **"테스트에서는 잘 나왔는데 배포하니
성능이 안 나온다"의 대부분이 이것이다.** 모델은 그대로인데 데이터가 달라진 것이다.

같은 데이터를 무작위로 쪼개서 잰 [테스트 점수](#train-test-split)는 **같은 분포에서만** 유효하다.
KLUE-YNAT에서는 학습 데이터의 `사회`가 11%인데 평가 데이터에서는 41%였고,
`사회`를 가장 못 맞히던 모델의 정확도가 0.8455에서 0.7639로 떨어졌다.
대응은 ① 카테고리별 약점을 미리 파악해두기, ② 배포 후 예측 분포를 감시하기, ③ 새 분포로 재학습.

#### <a id="text-vectorization"></a>`TextVectorization`
문자열을 **정수 시퀀스로 바꾸고 길이를 맞추는**(패딩) Keras 레이어. `adapt()`로 사전을 만든다.
사전의 0번은 패딩, 1번은 `[UNK]`(사전에 없는 단어)로 예약되어 있다.
`adapt()`는 **학습 데이터에만** 부른다 — 전체 데이터로 사전을 만들면 [데이터 누출](#data-leakage)이다.

#### <a id="padding-sequence"></a>패딩과 시퀀스 길이 (`output_sequence_length`)
신경망은 길이가 같은 배열을 받으므로, 짧은 문장은 0으로 채우고 긴 문장은 자른다.
길이는 **단어 수 분포의 95~99% 지점**으로 잡는다. 짧으면 뒤가 잘려 정보를 잃고,
길면 대부분이 0이라 계산만 낭비한다.

#### <a id="oov"></a>OOV / `[UNK]`
사전에 없는 단어(Out-Of-Vocabulary). 전부 `[UNK]` 한 칸으로 뭉개진다.
**한국어를 공백으로 자르면 이것이 심각해진다** — 뉴스 제목을 단어 단위로 넣었을 때
검증 데이터 토큰의 **38.5%가 `[UNK]`** 였다(`max_tokens=20000` 기준). `감시`는 사전에 있는데
`감시중`은 없는 식이다.

**사전을 무작정 키운다고 풀리지 않는다.** 어휘 전체(44,901개)를 담아 OOV를 31.5%로 낮춰도
정확도는 오르지 않았다. 한 번밖에 안 나온 단어의 임베딩은 학습될 수 없기 때문이다.
근본 대책은 **토큰 단위를 바꾸는 것**이다 — [문자 n-gram](#char-ngram)이나
`TextVectorization(split="character")`.

#### <a id="conv1d"></a>`Conv1D` (1차원 합성곱)
[CNN](#cnn)을 이미지가 아니라 **텍스트에 1차원으로** 적용한 것. 이미지에서 3×3 필터가 인접 픽셀을 보듯,
`Conv1D(128, 3)`은 **인접한 토큰 3개 묶음**에서 패턴을 찾는다.
**글자 단위 입력에 쓰면 사실상 학습되는 문자 3-gram이다** — 뉴스 제목 분류에서
단어 단위 `conv` 0.7487이 글자 단위로 바꾸자 0.7928이 됐다.

#### <a id="ensemble"></a>앙상블 (Ensemble)
여러 모델의 예측 확률을 평균 내어 최종 답을 정하는 방법.
**이득의 조건은 두 가지다.** ① 모델들이 서로 다른 실수를 할 것, ② **실력이 엇비슷할 것.**
둘 다 틀리는 행은 앙상블로도 구제되지 않고, 한쪽이 크게 약하면 평균이 오히려 발목을 잡는다
(뉴스 제목 분류에서 TF-IDF 0.8455와 신경망 0.7927을 섞자 **0.8225로 내려갔다**).

#### <a id="keras-h5"></a>`.h5` vs `.keras` (모델 저장 포맷)
`.h5`는 숫자 배열만 담는 옛 포맷이라 **[`TextVectorization`](#text-vectorization)이 들고 있는 단어 사전을
복원하지 못한다.** 고약한 것은 실패하는 시점이다 — **저장도 불러오기도 조용히 성공하고,
`predict()`를 부르는 순간 `FailedPreconditionError`가 난다.** 대응은 두 가지다 —
① `.keras`(최신 포맷)로 저장하거나, ② 벡터화를 모델 밖에 두고 사전을 따로(json 등) 저장한다.
**저장은 불러와서 예측이 되는지 확인할 때까지가 저장이다.**

## 8. 사전 학습 모델 (text-classification-practice 03)

이미 대량의 한국어로 훈련된 모델을 가져와 우리 문제에 맞추는 방법.
`notebooks/text-classification-practice/03_pretrained_korean`에서 KLUE-RoBERTa로 실습한다.

#### <a id="transfer-learning"></a>전이 학습 (Transfer Learning)
**한 곳에서 배운 것을 다른 문제에 옮겨 쓰는 것.** 텍스트 분류에서는 두 단계로 나뉜다 —
**사전 학습**(대량의 한국어로 언어 자체를 배움, 만든 사람이 이미 끝내둠)과
**파인튜닝**(우리 데이터로 분류만 가르침). 우리가 하는 것은 두 번째뿐이다.

**데이터가 적을수록 격차가 커진다.** 처음부터 배우는 모델은 16,000건으로
"한국어"와 "주제 분류"를 동시에 배워야 하는데, 그 양으로는 `정부는`과 `정부가`가
같은 말이라는 것조차 배울 수 없다. → [파인튜닝](#fine-tuning)

#### <a id="pretraining"></a>사전 학습 (Pre-training)
라벨 없이 **글 자체만으로** 언어를 배우는 단계. 문장의 일부를 가리고 맞히게 하는 식이라
정답을 사람이 붙일 필요가 없고, 그래서 인터넷 분량의 텍스트를 쓸 수 있다.
GPU 수십 대로 며칠 걸리는 일이며, **개인이 할 일이 아니다.** 공개된 것을 가져다 쓴다.

#### <a id="fine-tuning"></a>파인튜닝 (Fine-tuning)
사전 학습된 몸통 위에 **우리 문제용 머리**(분류 층)를 얹고, 우리 데이터로 조금 더 학습하는 것.
`AutoModelForSequenceClassification.from_pretrained(..., num_labels=7)`이 이 조립을 해준다.

**학습률을 아주 작게 잡는다**(2e-5~5e-5). 처음부터 배우는 것이 아니라 **이미 아는 것을 조금
조정**하는 것이라, 크게 움직이면 사전 학습으로 얻은 지식을 망가뜨린다(→ [치명적 망각](#catastrophic-forgetting)).
epoch도 2~4면 충분하다. 더 돌린다고 좋아지지 않는다.

#### <a id="catastrophic-forgetting"></a>치명적 망각 (Catastrophic Forgetting)
파인튜닝 중에 **사전 학습으로 얻은 것을 잃어버리는 현상.** 학습률이 너무 크면 일어난다.
증상은 [학습률](#learning-rate)이 클 때와 같아 보이지만(정확도가 안 오르거나 발산),
잃는 것이 다르다 — 처음부터 배우는 모델은 다시 배우면 그만이지만, 여기서는
**남이 며칠 걸려 만든 것을 망가뜨리는 것**이다.

#### <a id="subword"></a>서브워드 토큰화 (Subword Tokenization)
단어를 통째로 사전에 넣는 대신 **자주 나오는 조각**을 담아두고 단어를 조립하는 방식.
`정부는` → `정부` + `는`.

**한국어에서 특히 크게 이긴다.** 조사·어미가 붙어 `정부는`/`정부가`가 다른 단어가 되는 문제를
뿌리에서 없앤다. 같은 조각을 나눠 갖기 때문이다. [문자 n-gram](#char-ngram)이 01번에서
단어 단위를 이겼던 것과 **같은 원리**다.

실측: 검증 데이터의 [OOV](#oov)가 단어 단위 사전(20,000)에서 **38.8%**,
서브워드 사전(32,000)에서 **0.16%**. 대가는 토큰 수 증가 —
제목 하나가 6.6개에서 13.4개가 된다.

#### <a id="cls-token"></a>`[CLS]` 토큰
문장 맨 앞에 붙는 특별 토큰으로, **문장 전체의 요약이 담길 자리.** 분류 머리는 이 한 칸만 보고
답을 고른다. 02번의 `GlobalAveragePooling1D`(여러 벡터를 하나로 줄이기)가 하던 일을
이 토큰이 맡는다고 보면 된다.

#### <a id="attention-mask"></a>어텐션 마스크 (Attention Mask)
**어디까지가 진짜 문장인지** 알려주는 0/1 배열. 길이를 맞추려고 뒤에 채워 넣은 빈칸을
모델이 내용으로 착각하지 않게 한다. `1`은 진짜 토큰, `0`은 채운 자리.

#### <a id="newly-initialized"></a>`newly initialized` 경고
사전 학습 모델을 분류용으로 불러올 때 나오는 빨간 경고. **정상이다.**
원래 달려 있던 머리를 떼고 우리 문제용 머리를 새로 달았으니 "이 부분은 아직 아무것도 모른다"는
안내다. **다만 `classifier`가 아닌 층이 이 목록에 뜨면** 모델 이름을 잘못 적은 것이다.

#### <a id="model-tradeoff"></a>정확도 말고 봐야 하는 것
사전 학습 모델이 [TF-IDF](#tfidf)를 이겼다고 늘 그쪽을 고르는 것은 아니다.
같은 데이터에서 잰 비교다.

| | TF-IDF + 로지스틱 회귀 | 사전 학습 모델 |
|---|---|---|
| 검증 정확도 | 0.8455 | **0.8705** |
| 평가 정확도([분포 이동](#distribution-shift) 후) | 0.7639 | **0.8345** |
| 학습 시간(CPU) | 1분 안쪽 | 52분 |
| 모델 크기 | 수~수십 MB | 약 270MB (파라미터 6,800만 개) |
| **왜 그렇게 답했는지** | 단어별 계수를 직접 볼 수 있음 | 사실상 불가능 |

초당 수천 건을 분류해야 하거나, "왜 이렇게 분류됐냐"에 답해야 하는 자리라면 왼쪽이 낫다.
**라벨이 몇 백 건뿐일 때는 오른쪽이 압도적이다.**

#### <a id="shift-not-fixed"></a>모델을 바꿔도 안 되는 것
[분포 이동](#distribution-shift)에 대해 **"모델을 바꾸면 줄어든다"는 맞고
"없어진다"는 틀리다.**

같은 데이터에서 잰 결과다. 사전 학습 모델은 절대 성능이 높을 뿐 아니라
(평가 데이터 0.7639 → **0.8345**), 검증에서 평가로 넘어갈 때의 **낙폭도 절반 이하로 줄었다**
(−0.0816 → **−0.0360**). 훨씬 넓은 한국어를 보고 온 덕분에 처음 보는 표현에 덜 흔들린다.

**그래도 낙폭은 남는다.** −0.0360는 분할을 바꿀 때의 흔들림(0.01)보다 서너 배 크다.
데이터가 달라진 것이지 모델이 못난 것이 아니기 때문이다.
대응책(카테고리별 약점 파악, 예측 분포 감시, 재학습)은 모델을 바꿔도 그대로 필요하다.
...
**문장 전체를 한 번에 놓고 필요한 곳을 골라 본다.**

RNN을 대체한 이유는 속도보다 **병렬화**다. RNN은 t번째를 계산하려면 t−1번째가 끝나야 해서
시퀀스 길이만큼 차례를 기다리지만, Transformer는 그 계산을 **한 번의 행렬 곱**으로 한다.
그래서 모델을 크게 키울수록 이득이 커진다. 다만 글자 수 T에 대해 T×T 표를 만들므로
**긴 문장에서는 계산량 자체가 더 많다.** "더 싸다"가 아니라 "나눠서 할 수 있다"가 핵심이다.

#### <a id="attention"></a>어텐션 (Attention)
**지금 위치에서 어느 글자를 얼마나 볼지 정하고, 그 비율대로 섞는 것.**
"고른다"를 미분 가능하게 만들려고 가중 평균을 쓴다. 가중치를 한 곳에 몰면 하나를 집은 것과 같고,
고르게 퍼뜨리면 전체를 뭉뚱그려 본 것이 된다. **가중치가 곧 "어디를 볼지"다.**

#### <a id="self-attention"></a>셀프 어텐션 (Self-Attention)
[Q, K, V](#qkv)를 **모두 같은 입력에서** 만들어 문장이 자기 자신을 보게 하는 어텐션.
같은 글자 벡터에 서로 다른 가중치 행렬 세 개를 곱해 만든다.

#### <a id="qkv"></a>Q / K / V (Query, Key, Value)
어텐션 가중치를 계산하기 위한 세 가지 벡터. 도서관에 비유하면 —

| | 비유 | 하는 일 |
|---|---|---|
| **Query** | 사서에게 내미는 쪽지 | 지금 위치가 **무엇을 찾는가** |
| **Key** | 책등의 라벨 | 각 글자가 **자신을 뭐라고 광고하는가** |
| **Value** | 책의 내용 | 골랐을 때 **실제로 가져올 것** |

**Q와 K를 굳이 나누는 이유**: 찾는 쪽의 사정과 광고하는 쪽의 사정이 다르기 때문이다.
하나로 합치면 "나와 닮은 글자"밖에 못 찾는데, 실제로 필요한 것은 "나에게 필요한 것"이다.

#### <a id="scaled-dot-product-attention"></a>스케일드 닷 프로덕트 어텐션 (Scaled Dot-Product Attention)
어텐션의 표준 계산식. `softmax(QKᵀ / √d) · V` — 네 단계다.

1. `Q @ K.T` — 내적으로 점수를 낸다
2. `/ √d` — 차원 수 때문에 커진 크기를 되돌린다
3. `softmax` — 합이 1인 가중치로 바꾼다
4. `@ V` — 가중 평균

**√d로 나누는 이유**가 자주 오해된다. "확신을 낮추려는 것"이 아니라 **"차원 수 d가 커질수록
내적 값의 폭이 커지므로 원래 크기로 되돌리는 것"**이다. 안 나누면 softmax가 한 칸에 0.999를
몰아주고, 나머지 칸은 가중치가 0이라 [기울기](#gradient-descent)도 0이 되어 학습 신호가 흐르지 않는다.

#### <a id="causal-mask"></a>인과 마스크 (Causal Mask)
**미래를 못 보게 가리는 삼각형 표.** 다음 글자를 맞히는 것이 목표인데 어텐션은 문장 전체를
보므로, 그냥 두면 정답을 읽어와 버린다. 미래 쪽 점수를 `-inf`로 만들어 softmax 이후 0이 되게 한다.

**빼먹으면 loss가 비정상적으로 낮게 나온다.** [데이터 누출](#data-leakage)과 정확히 같은 종류의 사고이고,
증상도 같다 — 성능이 너무 좋아 보인다. 생성을 시켜보면 문장이 엉망이라 바로 들통난다.

#### <a id="multi-head-attention"></a>멀티 헤드 어텐션 (Multi-Head Attention)
어텐션을 여러 벌 동시에 돌리고 결과를 이어 붙이는 것. 가중치가 한 벌이면 한 자리에서
여러 곳을 봐야 할 때(누가 + 무엇을) 둘 중 하나에 쏠리거나 섞여 뭉개진다.

**차원을 나눠 쓴다**는 점이 중요하다. 64차원을 헤드 4개가 16차원씩 갖는다.
그래서 헤드를 늘려도 계산량이 늘지 않는다.

#### <a id="positional-encoding"></a>위치 인코딩 / 위치 임베딩 (Positional Encoding/Embedding)
**어텐션은 순서를 모른다.** 가중 평균에는 순서가 없어서, 단어를 섞어 넣어도 각 단어가
받는 출력이 완전히 같다. 그래서 "몇 번째 자리인지"를 벡터로 만들어 글자 벡터에 **더해준다.**

원래 논문은 사인/코사인 함수로 만들었지만(그래서 "인코딩"), GPT 계열은 위치마다 벡터 하나씩
두고 **학습시킨다**(`nn.Embedding(block_size, d_model)`). 대신 미리 만들어둔 자리 수를
넘길 수 없다 — 이것이 [컨텍스트 윈도우](#context-window)다.

#### <a id="context-window"></a>컨텍스트 윈도우 (Context Window)
모델이 **한 번에 볼 수 있는 토큰 수의 상한.** LLM 광고의 "128K 컨텍스트"가 이 숫자다.
[위치 임베딩](#positional-encoding)을 그만큼만 만들어뒀기 때문에 생기는 제약이다.

#### <a id="residual-connection"></a>잔차 연결 (Residual Connection)
`x + f(x)` 형태로 **원본을 그대로 옆으로 흘려보내는 것.** 층을 깊게 쌓을 수 있게 해준다.

[역전파](#backpropagation)는 곱셈의 연쇄라 층이 깊어지면 기울기가 0으로 사그라드는데
(→ [기울기 소실](#vanishing-gradient)), `+x` 덕분에 **1이 하나 더해진 경로**가 생겨 그 일이 안 벌어진다.

#### <a id="layer-norm"></a>LayerNorm (Layer Normalization)
**벡터 하나 안의 값 분포를 평균 0, 표준편차 1로 맞추는 것.** 없으면 층을 지날수록 값이
커지거나 작아져 학습이 불안정해진다.

[스케일링](#scaling)과 목적은 같지만 대상이 다르다 — 스케일링은 데이터 열 단위,
LayerNorm은 **샘플 하나의 벡터 안에서** 맞춘다.

#### <a id="transformer-block"></a>Transformer 블록
`LayerNorm → 어텐션 → (+원본) → LayerNorm → FFN → (+원본)` 한 묶음.
**입력과 출력의 크기가 같아서** 원하는 만큼 쌓을 수 있다. GPT-3은 96층, 실습용 TinyGPT는 2층인데
**구조는 똑같다.**

LayerNorm을 어텐션 **앞**에 두는 것(Pre-LN)이 요즘 방식이다. 원래 논문은 뒤에 뒀지만
앞이 학습이 훨씬 안정적이라 GPT 계열은 전부 앞에 둔다.

#### <a id="ffn"></a>FFN (Feed-Forward Network)
Transformer 블록 안의 2층 신경망(`Linear → ReLU → Linear`). 어텐션이 **"주변에서 정보를
모아오는"** 일이라면, FFN은 **"모아온 것으로 생각하는"** 일이다.
안쪽을 4배로 넓혔다 줄이는 것이 관례다.

#### <a id="next-token-prediction"></a>다음 토큰 예측 (Next Token Prediction)
LLM의 학습 목표. **입력 `x`와 정답 `y`가 한 칸 어긋난 같은 문장**이다.
...
문장 하나로 위치마다 문제가 하나씩, 총 T개가 동시에 만들어진다. **사람이 정답을 붙일 필요가
없다는 것이 핵심이다.** 글만 있으면 되므로 인터넷 분량의 텍스트를 쓸 수 있고,
그래서 [사전 학습](#pretraining)이 가능하다.

마지막 층의 출력 크기가 어휘 수라, 사실 [소프트맥스 회귀](#softmax-regression)와 같은 다중 분류 문제다.

#### <a id="induction-head"></a>유도 헤드 (Induction Head)
**"아까 이런 자리에서 뭐가 나왔더라?"를 담당하는 어텐션 헤드.**
`... A B ... A` 를 보면 다음이 `B`일 것이라고 앞을 되짚어 찾아온다.

두 층이 있어야 생긴다 — 1층이 "내 앞에 뭐가 있었는지"를 적어두고, 2층이 그것을 단서로
찾는다. **층을 쌓는 이유를 눈으로 확인할 수 있는 가장 좋은 예**이고, 실습에서 1층 모델이
되풀이(복사)를 못 하는 것으로 확인한다.

#### <a id="temperature"></a>온도 (Temperature)
생성할 때 확률을 뽑기 전에 점수를 나누는 값. LLM API의 `temperature` 파라미터가 이것이다.
낮으면 1등에 쏠려 **뻔해지고**, 높으면 골고루 뽑혀 **엉뚱해진다.**

| 하려는 일 | 온도 |
|---|---|
| 사실 추출, 분류, [정형 출력](#structured-output) | 낮게 (0~0.3) |
| 요약, 번역 | 중간 (0.3~0.7) |
| 아이디어 내기 | 높게 (0.8~1.2) |

**온도를 0으로 해도 정답이 보장되지는 않는다.** 온도는 다양성만 정한다.
모델이 잘못 알고 있으면 그 틀린 답을 아주 일관되게 내놓을 뿐이다.

#### <a id="llm"></a>대규모 언어 모델 (LLM, Large Language Model)
...
요즘 LLM이 쓰는 위치 표현 방식. 위치 벡터를 **더하는** 대신 Q와 K를 **회전**시켜
두 글자 사이의 **상대적인 거리**가 자연스럽게 반영되게 한다.
학습 때 본 것보다 긴 문장에 상대적으로 잘 대응한다.
