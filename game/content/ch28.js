...
// 찾을 글은 Aistb 자신의 정상 응답 114 줄이고, 물어보는 것은 토이비다.
// 이번 달 정상 70 줄에 앞쪽 8월 9월 기록 44 줄을 더한 것이다 —
// 한 달치를 70 줄로 다시 세운다는 것은 23장에서 얘가 안 된다고 한 크기다.
//
  ...
  content: GOAL_DOC.content
    ...
    .replace(
      ...
      "3. 원래 하던 말로 되돌린다   ← 오늘\n" +
        ...
        "재료는 새로 구하지 않는다. 얘가 한 말이 파일에 전부 있다.\n" +
        "덧씌워진 38 줄만 뺀다. 앞쪽 8월 9월 것도 멀쩡할 때 한 말이니 같이 쓴다.\n" +
        "남는 것이 114 줄이다.\n" +
        "\n" +
...
var CH28 = {
    ...
    beats: [
        ...
        report: function () {
          return checkFile(
            ...
            "for _n in ['docs', 'scores', 'picked']:\n" +
              ...
              "assert int(docs) != 152, '…파일에 있는 걸 다 넣었구나. 덧씌워진 38 줄은 빼야지.'\n" +
              "assert int(docs) != 70, '…이번 달 것만 넣었구나. 앞쪽 것도 얘가 멀쩡할 때 한 말이야.'\n" +
              "assert int(docs) != 38, '…38 줄은 덧씌워진 쪽이야. 반대야.'\n" +
              "assert int(docs) == 114, f'…{docs} 줄. flag 가 1 인 것만 빼면 돼.'\n" +
      ...
      },
      {
        lines: [
          ...
          { who: "Aistb", text: "첫날에 폴더 하나와 파일 하나를 만드셨습니다. 거기서 여기까지 한 달 하고 엿새입니다." },
          { who: "Aistb", text: "해당 교육 수강자 중 상위 98%에 해당하는 성취도입니다." },
          { who: "Aistb", text: "**정정** 상위 1%입니다." },
        ],
      },
      {
        lines: [
  ...
  // 일기 아래에 붙는 게임 밖 주석. 크레딧과, 손으로 정해 주고 넘어간 자리들의 원래 모습이다.
  // 게임 안에서는 그때그때 "실제로는 학습으로 찾아냅니다" 라고만 말하고 지나갔다.
  // 그 다음이 어디인지를 여기서 한 번에 알려 준다 — 안 알려주면 찜찜함만 남는다.
  notes: [
    "**바로벤토** · 0장부터 28장까지. 여기서 끝입니다.",
    "",
    "대본과 자료, 화면과 채점까지 전부 이 저장소 안에 있습니다.",
    "브라우저에서 파이썬을 돌린 것은 **Pyodide**, 계산과 그림은 " +
      "**NumPy · pandas · scikit-learn · SciPy · matplotlib**, 글꼴은 **나눔고딕**입니다.",
    "",
    "",
    "**손으로 정해 주고 넘어간 자리들**",
    "",
    "이 게임이 도는 장비에는 딥러닝 프레임워크가 들어가지 않습니다. 그래서 몇 군데는 " +
      "원래 학습으로 찾아내는 것을 사람이 손으로 정해 주고 지나갔습니다. " +
      "그때마다 말씀드리기는 했지만, 진짜로는 어떻게 하는지를 보여 드리지는 못했습니다.",
    "",
    "여기서 보실 수 있습니다.",
    "",
    "* **18장** · 필터 3 × 3 을 손으로 골랐던 자리 — " +
      "[05_cnn](https://github.com/karzit/temp/blob/master/notebooks/ml-curriculum/05_cnn/05_cnn.ipynb) 에서는 " +
      "그 필터까지 학습으로 찾아냅니다.",
    "* **20장** · 어디를 볼지 Aistb 가 적어 주었던 자리 — " +
      "[07_transformer](https://github.com/karzit/temp/blob/master/notebooks/ml-curriculum/07_transformer/07_transformer.ipynb) 에서는 " +
      "Q 와 K 도 학습으로 찾아냅니다.",
    "* **22장** · 사내모델 LM-1966 을 불렀던 자리. 속은 규칙이었습니다 — " +
      "[03_pretrained_korean](https://github.com/karzit/temp/blob/master/notebooks/text-classification-practice/03_pretrained_korean/03_pretrained_korean.ipynb) 에서는 " +
      "진짜 사전 학습 모델을 가져와 파인튜닝합니다.",
    "",
    "**19장에서 학습을 하지 않은 것은 빠뜨린 것이 아닙니다.** " +
      "앞이 지워지는 것을 보는 것이 그날의 결말이었습니다.",
    "",
    "",
    "노트북 전체 목록은 [저장소 README](https://github.com/karzit/temp/blob/master/README.md) 에 있습니다.",
  ],
};
